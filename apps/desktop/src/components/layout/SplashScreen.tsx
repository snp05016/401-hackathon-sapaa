...
export function SplashScreen({ exiting, onSkip }: { exiting: boolean; onSkip: () => void }) {
  return (
    <div
      ...
      onClick={onSkip}
      className={cn(
        "editorial fixed inset-0 z-50 flex flex-col items-center justify-center will-change-transform",
        exiting && "animate-curtain",
      )}
    >
      <GhostMark size={30} className="animate-mark-in mb-7 text-oxblood" />

      <h1 className="flex font-display text-[76px] leading-none tracking-[-0.02em] text-ink">
      ...
      <div
        className="animate-draw-rule mt-7 h-px w-[340px] origin-center bg-hairline"
